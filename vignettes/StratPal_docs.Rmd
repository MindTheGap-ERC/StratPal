...
* the second element is names `vals`: a list with the same number of elements as the vector `h`/`t`. Each element of the list contains the trait values of specimens sampled at that time/height.

### Construction

`pre_paleoTS` is constructed by functions to simulate trait evolution on a specimen level/in pre-paleoTS format. These functions end in `_sl` for "specimen level". This is
...
No standalone plotting procedure is implemented for `pre_paleoTS`. For plotting, first use `reduce_to_paleoTS` and plot the results via `plot.paleoTS` from the `paleoTS` package.

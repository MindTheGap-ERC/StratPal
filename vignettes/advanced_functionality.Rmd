...
We used a simple niche definition based on a probability density function of a normal distribution implemented in `cnd`. This can be expanded to arbitrary niche definitions along a gradient. For integration with `apply_niche_pref`, niche definitions must meet the following criteria:
...
As long as these conditions for `gc` and `niche_def` are met, arbitrary niche preferences along any gradient can be modeled using `apply_niche_pref`.
